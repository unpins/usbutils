 ...
 *
 * unpins: this is the historical text-file parser for usb.ids (the modern
 * upstream names.c resolves names only via udev's binary hwdb, which has no
 * Windows/macOS analogue). It is restored here and fed the usb.ids database
 * *embedded into the binary* (usb_ids.h, generated by `xxd -i` at build time
 * from hwdata's usb.ids) so lsusb resolves vendor / product / class names with
 * NO companion file on every OS -- a true single binary, mirroring how the
 * unpins pciutils embeds pci.ids. The on-disk file reader is replaced by a
 * portable byte-buffer line reader (no fmemopen, which mingw lacks).
 */
...
struct usb_id_stream {
	const unsigned char *buf;
	size_t len;
	size_t pos;
};
typedef struct usb_id_stream *usb_file;

static char *usb_id_gets(char *line, int size, usb_file s)
{
	int i;
	if (s->pos >= s->len)
		return NULL;
	for (i = 0; i < size - 1 && s->pos < s->len; ) {
...
#define usb_fgets(s, size, stream)	usb_id_gets(s, size, stream)
...
/*
 * Try the embedded usb.ids first (idVendor/idProduct -> friendly names); when a
 * name is missing, fall back to the device's own string descriptors read from
 * sysfs (Linux only -- get_sysfs_name/read_sysfs_prop are no-ops elsewhere).
 */
...
int names_init(void)
{
	struct usb_id_stream s = { unpin_usb_ids, unpin_usb_ids_len, 0 };
